(() => {
  ...
  function lockBoard(disabled=true){
    boardEl.querySelectorAll('.tile').forEach(t=>{
      ...
      if(disabled){ t.tabIndex = -1; } else { t.tabIndex = 0; }
    });
    over = disabled;
  }
  ...
  function onClickTile(e){
    ...
    if(used >= LIMIT){
      setMessage(`😔 Game Over — You've used all ${LIMIT} chances. Nemo was here →`, 'lose');
  ...
  function newGame(){
    ...
    updateCounters();
    lockBoard(false);
    setMessage('Find Nemo hidden under a shell. You have only 5 chances! <span class="small">Tip: You can use keyboard arrows + Enter to play.</span>');
  }
